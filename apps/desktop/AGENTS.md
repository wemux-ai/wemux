...
- Persists the selected control-plane origin in Electron user data and loads it as the top-level page so authentication remains same-origin.
...
- Only load the bundled `wemux-app://local` origin or an explicitly selected HTTP(S) control-plane origin in the application window.
