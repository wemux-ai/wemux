...
export type EnsureEasyTierBinariesOptions = {
  ...
  ) => { ok: boolean; stderr: string; error?: string }
}
...
export const resolveEasyTierInstallDir = (params: {
  ...
  const platform = resolveEasyTierPlatform(params.platform)
  const arch = resolveEasyTierArch(params.arch)
  return join(params.runtimeRoot?.trim() || getRuntimeRoot(), 'easytier', version, `${platform}-${arch}`)
}
...
const runUnzip = (
  archivePath: string,
  installDir: string,
  options: EnsureEasyTierBinariesOptions,
) => {
  const unzipPath = (options.resolveExecutable ?? (() => ''))('unzip')
  if (!unzipPath) {
  ...
  const result = options.runCommand?.(unzipPath, ['-oq', archivePath, '-d', installDir], { timeout: 120000 })
  if (!result) {
    throw new Error('EasyTier unzip command runner is unavailable.')
  }
  if (!result.ok) {
    throw new Error(result.stderr || result.error || 'EasyTier archive extraction failed.')
  }
}
...
export const ensureEasyTierBinaries = async (options: EnsureEasyTierBinariesOptions = {}): Promise<EasyTierBinarySet> => {
  ...
  runUnzip(archivePath, installDir, options)
